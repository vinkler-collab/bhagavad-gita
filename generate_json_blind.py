    ...
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                # Tagy (původní ruční tagy)
                tags_match = re.search(r'\{tags:\s*(.*?)\}', raw_line, re.IGNORECASE)
                if tags_match:
                    if metadata["tags"] is None:
                        metadata["tags"] = []
                    metadata["tags"].extend([t.strip() for t in tags_match.group(1).split(',') if t.strip()])
                    continue

                # --- NOVINKA: AUTOMATICKÁ EXTRAKCE VERŠŮ ZE {sov: ...} ---
                # Najde např. "1.2" uvnitř "{sov: Verse 1.2}" nebo "{sov: 1-2}"
                sov_matches = re.finditer(r'\{sov:[^}]*?([0-9]+[.\-][0-9]+)[^}]*\}', raw_line, re.IGNORECASE)
                for match in sov_matches:
                    verse_number = match.group(1).strip()
                    if metadata["tags"] is None:
                        metadata["tags"] = []
                    if verse_number not in metadata["tags"]:
                        metadata["tags"].append(verse_number)

                # AUDIO - hledáme {audio: cesta | popisek} nebo jen {audio: cesta}
